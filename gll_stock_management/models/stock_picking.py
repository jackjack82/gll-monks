...
class StockPicking(models.Model):
    ...
    # Servizi Logistici fields
    servizi_logistici = fields.Float(
        string="Servizi logistici",
        compute="_compute_servizi_logistici",
        store=True,
        help="Somma di tutte le righe d'ordine con prodotti aventi tipologia warehouse",
    )
    diritto_fisso = fields.Float(
        string="Diritto fisso",
        compute="_compute_servizi_logistici",
        store=True,
        help="Somma di tutte le righe d'ordine con prodotti aventi tipologia subscription",
    )
    preparazione = fields.Float(
        string="Preparazione",
        compute="_compute_servizi_logistici",
        store=True,
        help="Somma di tutte le righe d'ordine con prodotti collegati ai parametri di configurazione",
    )
    totale_logistico = fields.Float(
        string="Totale logistico",
        compute="_compute_servizi_logistici",
        store=True,
        help="Somma dei tre campi: Servizi logistici + Diritto fisso + Preparazione",
    )

    # Servizi di Trasporto Italia fields
    transport_service_total = fields.Float(
        string="Transport",
        compute="_compute_servizi_trasporto",
        store=True,
    )
    accessories_service_total = fields.Float(
        string="Accessories",
        compute="_compute_servizi_trasporto",
        store=True,
    )
    additional_service_total = fields.Float(
        string="Additional",
        compute="_compute_servizi_trasporto",
        store=True,
    )
    fixed_service_total = fields.Float(
        string="Fixed",
        compute="_compute_servizi_trasporto",
        store=True,
    )
    variable_service_total = fields.Float(
        string="Variable",
        compute="_compute_servizi_trasporto",
        store=True,
    )
    totale_trasporto = fields.Float(
        string="Totale trasporto",
        compute="_compute_servizi_trasporto",
        store=True,
        help="Somma di tutti i valori calcolati in questo gruppo",
    )

    # Altri Servizi fields
    altri_servizi = fields.Float(
        string="Altri servizi",
        compute="_compute_altri_servizi",
        store=True,
        help="Somma di tutte le righe d'ordine non comprese nei gruppi precedenti",
    )
    totale_altri_servizi = fields.Float(
        string="Totale altri servizi",
        compute="_compute_altri_servizi",
        store=True,
        help="Somma di queste righe",
    )
    ...
    @api.depends("all_service_ids.product_id.pick_service_type", "all_service_ids.total")
    def _compute_servizi_logistici(self):
        """Compute the fields for the 'Servizi Logistici' group"""
        for picking in self:
            # Servizi logistici: sum of order lines with products having pick_service_type = 'warehouse'
            picking.servizi_logistici = sum(
                service.total
                for service in picking.all_service_ids
                if service.product_id.product_tmpl_id.pick_service_type == 'warehouse'
            )

            # Diritto fisso: sum of order lines with products having pick_service_type = 'subscription'
            picking.diritto_fisso = sum(
                service.total
                for service in picking.all_service_ids
                if service.product_id.product_tmpl_id.pick_service_type == 'subscription'
            )

            # Preparazione: sum of order lines with products linked to service_for_box_id and service_for_single_id
            service_for_single_id, service_for_box_id = picking.get_single_box_product_services()
            picking.preparazione = sum(
                service.total
                for service in picking.all_service_ids
            ...
            picking.totale_logistico = picking.servizi_logistici + picking.diritto_fisso + picking.preparazione

    @api.depends("all_service_ids.product_id.pick_service_type", "all_service_ids.total")
    def _compute_servizi_trasporto(self):
        """Compute the fields for the 'Servizi di Trasporto Italia' group"""
        for picking in self:
            # Sum for each pick_service_type different from 'warehouse' and 'subscription'
            transport_types = ['transport', 'accessories', 'additional', 'fixed', 'variable']

            picking.transport_service_total = sum(
                service.total
                for service in picking.all_service_ids
                if service.product_id.product_tmpl_id.pick_service_type == 'transport'
            )

            picking.accessories_service_total = sum(
                service.total
                for service in picking.all_service_ids
                if service.product_id.product_tmpl_id.pick_service_type == 'accessories'
            )

            picking.additional_service_total = sum(
                service.total
                for service in picking.all_service_ids
                if service.product_id.product_tmpl_id.pick_service_type == 'additional'
            )

            picking.fixed_service_total = sum(
                service.total
                for service in picking.all_service_ids
                if service.product_id.product_tmpl_id.pick_service_type == 'fixed'
            )

            picking.variable_service_total = sum(
                service.total
                for service in picking.all_service_ids
                if service.product_id.product_tmpl_id.pick_service_type == 'variable'
            )

            # Totale trasporto: sum of all values in this group
            picking.totale_trasporto = (
                picking.transport_service_total +
                picking.accessories_service_total +
                picking.additional_service_total +
                picking.fixed_service_total +
                picking.variable_service_total
            )

    @api.depends("all_service_ids.product_id.pick_service_type", "all_service_ids.total")
    def _compute_altri_servizi(self):
        """Compute the fields for the 'Altri Servizi' group"""
        for picking in self:
            # Altri servizi: sum of all order lines not included in the previous groups
            # (products without any specified pick_service_type)
            picking.altri_servizi = sum(
                service.total
                for service in picking.all_service_ids
                if not service.product_id.product_tmpl_id.pick_service_type
            )

            # Totale altri servizi: sum of these lines
            picking.totale_altri_servizi = picking.altri_servizi
