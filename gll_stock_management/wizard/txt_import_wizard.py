...
class TxtImportWizard(models.Model):
        ...
        try:
            ...
            for line in lines:
                ...
                try:
                    ...
                    shipment_date_str = line[20:29].strip()
                    origin_doc = line[29:79].strip()
                    customer_name = line[88:122].strip()
                    customer_street = line[122:152].strip()
                    customer_zip = line[152:157].strip()
                    customer_state_code = line[157:159].strip()
                    customer_city = line[159:189].strip()
                    recipient_name = line[189:234].strip()
                    recipient_street = line[234:269].strip()
                    recipient_zip = line[269:275].strip()
                    recipient_state_code = line[276:279].strip()
                    recipient_city = line[279:309].strip()
                    product_ref = line[413:428].strip()
                    ...
                    # Process the data
                    self._process_line(
                        shipment_id, shipment_date, origin_doc, 
                        customer_name, customer_street, customer_zip, customer_state_code, customer_city,
                        recipient_name, recipient_street, recipient_zip, recipient_state_code, recipient_city,
                        product_ref, processed_pickings, result_message
                    )
                    
                except Exception as e:
                    result_message.append(f"Error processing line: {str(e)}")
    ...
    def _process_line(self, shipment_id, shipment_date, origin_doc, 
                     customer_name, customer_street, customer_zip, customer_state_code, customer_city,
                     recipient_name, recipient_street, recipient_zip, recipient_state_code, recipient_city,
                     product_ref, processed_pickings, result_message):
        ...
        if shipment_id in processed_pickings:
            ...
            result_message.append(f"Adding product {product_ref} to existing picking {picking.name}")
        else:
            # Find or create the customer
            customer = self._find_or_create_partner(
                customer_name, customer_street, customer_zip, 
                customer_state_code, customer_city, result_message
            )
            
            # Find or create the recipient
            recipient = self._find_or_create_partner(
                recipient_name, recipient_street, recipient_zip, 
                recipient_state_code, recipient_city, result_message
            )
            ...
            vals = {
                ...
                'delivery_partner_id': recipient.id,
            ...
            result_message.append(f"Created new picking {picking.name} for shipment {shipment_id}")
    ...
    def _find_or_create_partner(self, name, street, zip_code, state_code, city, result_message):
        ...
        if not partner:
            ...
            result_message.append(f"Created new partner: {name}")
    ...
    def _add_product_to_picking(self, picking, product_ref, result_message):
        ...
        if not product_ref:
            result_message.append("Warning: Empty product reference, skipping")
            return
        ...
        if not product:
            result_message.append(f"Warning: Product with reference {product_ref} not found, skipping")
            return
        ...
        result_message.append(f"Added product {product_ref} to picking {picking.name}")